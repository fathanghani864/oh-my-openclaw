...
async function createTempPreset(
  ...
  const presetDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'skills-test-preset-')
  );
...
async function createTempTarget(): Promise<string> {
  const targetDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'skills-test-target-')
  );
...
describe('copySkills', () => {
  test('copies skill directory to target', async () => {
    const presetDir = await createTempPreset('test-skill', {
      'SKILL.md': '# Test Skill',
    });
  ...
  test('creates target parent directory if missing', async () => {
    const presetDir = await createTempPreset('new-skill', {
      'SKILL.md': '# New Skill',
    });
  ...
  test('overwrites existing skill when force is true', async () => {
    const presetDir = await createTempPreset('update-skill', {
    ...
    await copySkills(presetDir, ['update-skill'], {
      force: true,
      targetBaseDir,
    });
  ...
  test('skips existing skill in non-TTY without force', async () => {
    const presetDir = await createTempPreset('existing-skill', {
      'SKILL.md': '# New Content',
    });
    const targetBaseDir = await createTempTarget();
  ...
  test('does not copy in dry-run mode', async () => {
    const presetDir = await createTempPreset('dry-skill', {
      'SKILL.md': '# Dry Run Skill',
    });
    const targetBaseDir = await createTempTarget();

    const logs = await captureLogs(async () => {
      await copySkills(presetDir, ['dry-skill'], {
        dryRun: true,
        targetBaseDir,
      });
    });
  ...
  test('installs multiple skills in order', async () => {
    const presetDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'skills-test-multi-')
    );
    tempDirs.push(presetDir);
    ...
    await copySkills(presetDir, ['skill-a', 'skill-b'], {
      force: true,
      targetBaseDir,
    });
    ...
    for (const name of ['skill-a', 'skill-b']) {
      const content = await fs.readFile(
        path.join(targetBaseDir, name, 'SKILL.md'),
        'utf-8'
      );
