...
## 이것은 무엇인가요?
apex는 셀프 호스팅 AI 에이전트 게이트웨이인 [OpenClaw](https://github.com/minpeter/openclaw)의 설정 프리셋을 관리하는 CLI 유틸리티입니다. `openclaw.json` 오버라이드와 워크스페이스 마크다운 파일을 번들로 묶어, 단일 명령으로 다양한 에이전트 성격, 도구 세트, 모델 구성 간에 전환할 수 있습니다.
...
3. 바이너리를 빌드합니다:
   ...
   ```
4. `dist/apex`에 생성된 바이너리를 PATH에 추가하거나 직접 실행합니다.

### 기본 워크플로우
1. **목록 조회** 사용 가능한 프리셋 확인: `apex list`
2. **비교** 현재 설정과 프리셋 비교: `apex diff apex`
3. **적용** 프리셋 적용: `apex apply apex`
4. **설치** apex 빠른 설치: `apex install`
5. **내보내기** 현재 설정을 새 프리셋으로 저장: `apex export my-custom-setup`
6. **적용** GitHub에서 프리셋 적용: `apex apply minpeter/demo-researcher`
...
### 딥 머지 의미론
프리셋을 적용할 때, apex는 `openclaw.json`에 대해 딥 머지 전략을 사용합니다:
...
### 자동 백업
변경 사항을 적용하기 전에, apex는 `~/.openclaw/apex/backups/`에 타임스탬프가 포함된 백업을 생성합니다 (`openclaw.json` 및 워크스페이스 파일 대체 시 워크스페이스 백업 포함).

## 커스텀 프리셋 만들기
프리셋은 `~/.openclaw/apex/`에 저장됩니다. `preset.json5` 파일과 관련 마크다운 파일(`AGENTS.md`, `SOUL.md` 등)이 포함된 디렉토리를 생성하여 수동으로 만들 수 있습니다.
...
원격 프리셋은 자동으로 `~/.openclaw/apex/presets/owner--repo/`에 사용자 프리셋으로 캐시됩니다. 이후 적용 시 `--force`를 지정하지 않으면 캐시된 버전을 재사용합니다.
